...
// PATCHED (see NimBLEDevice::deinit() below) - not upstream, will be lost if this dependency is
// reinstalled/updated. Tracked against https://github.com/h2zero/NimBLE-Arduino/issues/803.
...
# endif
        ...
        rc = nimble_port_stop();
        if (rc == 0) {
            // PATCHED - nimble_port_stop()'s two-phase semaphore handshake only confirms the host
            // task (a separate FreeRTOS task, possibly pinned to the other core) has *posted* that
            // it processed the stop event - not that it has finished unwinding out of
            // nimble_port_run(), returning into host_task(), and self-deleting via
            // nimble_port_freertos_deinit()/vTaskDelete(). Calling nimble_port_deinit() (which
            // disables/frees the BT controller) here can race that still-in-flight self-delete on
            // the other core, observed in the field as a "Guru Meditation Error ...
            // InstrFetchProhibited" panic with PC=0, backtrace landing inside host_task(). Matches
            // https://github.com/h2zero/NimBLE-Arduino/issues/803, unresolved upstream as of 2.5.1
            // - this delay is the same workaround reported there to alleviate it.
